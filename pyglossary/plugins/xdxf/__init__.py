...
class Reader:
	...
	def close(self) -> None:
		# TODO Lenny remove stats
		from pyglossary.plugins.xdxf.wiedza import abrewiatury
		abbra = [a for a, b, c, d in abrewiatury]
		abbrb = [b for a, b, c, d in abrewiatury]
		ddd = self._htmlTr.abbr_stats
		for w in sorted(ddd, key=ddd.get, reverse=True):
			if ddd[w] > 10 and w not in abbra + abbrb:
				print(ddd[w], w)
